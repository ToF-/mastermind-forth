REQUIRE mastermind.fs
REQUIRE ffl/tst.fs

PAGE
T{ ." some constants" CR
    MAX-COLORS 6 ?S
    MAX-PEGS 4 ?S
    MAX-CODEWORDS 1296 ?S
}T

T{ ." index to codeword and back" CR
    0 INDEX>CODEWORD 1111 ?S
    1111 CODEWORD>INDEX 0 ?S
    1295 INDEX>CODEWORD 6666 ?S
    6666 CODEWORD>INDEX 1295 ?S
}T

T{ ." code sets" CR
    CODE-SET MY-SET
    MY-SET INIT-SET
    MY-SET SET-SIZE 1296 ?S
}T

T{ ." emptying and inserting into set" CR
    MY-SET EMPTY-SET
    MY-SET SET-SIZE 0 ?S
    2611 MY-SET INSERT-CODEWORD
    3666 MY-SET INSERT-CODEWORD
    1115 MY-SET INSERT-CODEWORD
    MY-SET SET-SIZE 3 ?S
}T

T{ ." removing from a set" CR
    MY-SET INIT-SET
    MY-SET SET-SIZE 1296 ?S
    2611 MY-SET REMOVE-CODEWORD
    3666 MY-SET REMOVE-CODEWORD
    1115 MY-SET REMOVE-CODEWORD
    MY-SET SET-SIZE 1293 ?S
}T

T{ ." matching a codeword to a test" CR
    4263 1111 MATCHES 0 ?S
    4263 2433 MATCHES 1 ?S
    4263 3266 MATCHES 2 ?S
    4263 4263 MATCHES 4 ?S

    4263 1111 HITS 0 ?S
    4263 6211 HITS 2 ?S
    4263 3336 HITS 2 ?S
    4432 4111 HITS 1 ?S
    4263 3624 HITS 4 ?S

    4263 1111 MISSES 0 ?S
    4263 4236 MISSES 2 ?S
    4263 2111 MISSES 1 ?S
    4263 3331 MISSES 1 ?S
    4263 4263 MISSES 0 ?S


    4263 1111 MATCH 00 ?S
    4263 1122 MATCH 10 ?S
    4263 4111 MATCH 01 ?S
    4263 3264 MATCH 22 ?S
    4263 4263 MATCH 04 ?S
}T

T{ ." all codewords matching a codeword for a given result" CR
    MY-SET INIT-SET
    CODE-SET MY-RESULT
    1122 01 MY-SET MY-RESULT RESULT-MATCH-SET
    MY-RESULT SET-SIZE 256 ?S
    MY-RESULT START-SET
    MY-RESULT NEXT-CODEWORD 1333 ?S
    MY-RESULT NEXT-CODEWORD 1334 ?S
    MY-RESULT NEXT-CODEWORD 1335 ?S
}T

T{ ." copying a set" CR
    MY-SET INIT-SET
    MY-SET SET-SIZE 1296 ?S
    MY-RESULT EMPTY-SET
    1136 MY-RESULT INSERT-CODEWORD
    6631 MY-RESULT INSERT-CODEWORD
    MY-RESULT MY-SET COPY-SET
    MY-SET SET-SIZE 2 ?S
    MY-SET START-SET
    MY-SET NEXT-CODEWORD 1136 ?S
    MY-SET NEXT-CODEWORD 6631 ?S
}T

T{ ." match result table" CR
    0 RESULT-VALUE 04 ?S
    1 RESULT-VALUE 03 ?S
    2 RESULT-VALUE 02 ?S
    3 RESULT-VALUE 01 ?S
    4 RESULT-VALUE 00 ?S
    5 RESULT-VALUE 13 ?S
    6 RESULT-VALUE 12 ?S
    7 RESULT-VALUE 11 ?S
    8 RESULT-VALUE 10 ?S
    9 RESULT-VALUE 22 ?S
    10 RESULT-VALUE 21 ?S
    11 RESULT-VALUE 20 ?S
    12 RESULT-VALUE 31 ?S
    13 RESULT-VALUE 30 ?S
    14 RESULT-VALUE 40 ?S
}T

T{ ." matching a codeword on all possible results" CR
    MY-SET INIT-SET
    1122 MY-SET ALL-RESULTS-MATCH
    0 RESULT-VALUE . ." : " 0 RESULT-SET SET-SIZE . CR 0 RESULT-SET .SET CR
    1 RESULT-VALUE . ." : " 1 RESULT-SET SET-SIZE . CR 1 RESULT-SET .SET CR
    2 RESULT-VALUE . ." : " 2 RESULT-SET SET-SIZE . CR 2 RESULT-SET .SET CR
    3 RESULT-VALUE . ." : " 3 RESULT-SET SET-SIZE . CR 3 RESULT-SET .SET CR
    4 RESULT-VALUE . ." : " 4 RESULT-SET SET-SIZE . CR 4 RESULT-SET .SET CR
    5 RESULT-VALUE . ." : " 5 RESULT-SET SET-SIZE . CR 5 RESULT-SET .SET CR
    6 RESULT-VALUE . ." : " 6 RESULT-SET SET-SIZE . CR 6 RESULT-SET .SET CR

}T
BYE

T{ ." result index" CR
    20 RESULT-INDEX 11 ?S
    40 RESULT-INDEX 14 ?S
    03 RESULT-INDEX 1 ?S
}T

T{ ." codewords set" CR
    CODEWORD-SET MY-SET
    MY-SET INIT-SET
    MY-SET START-SET
    MY-SET NEXT-CODEWORD 1111 ?S
    MY-SET NEXT-CODEWORD 1112 ?S
    MAXCODEWORDS 3 - MY-SET !
    MY-SET NEXT-CODEWORD 6665 ?S
    MY-SET NEXT-CODEWORD 6666 ?S
    MY-SET NEXT-CODEWORD 0 ?S
}T

T{ ." eliminating codewords from set" CR
    1112 MY-SET ELIMINATE-CODEWORD
    MY-SET CELL+ 1+ SETSIZE 1- ERASE ( eliminate codewords 1123 to 6666 )
    MY-SET START-SET
    MY-SET NEXT-CODEWORD 1111 ?S
    MY-SET NEXT-CODEWORD 1113 ?S
    MY-SET NEXT-CODEWORD 1114 ?S
    MY-SET NEXT-CODEWORD 1115 ?S
    MY-SET NEXT-CODEWORD 1116 ?S
    MY-SET NEXT-CODEWORD 1121 ?S
    MY-SET NEXT-CODEWORD 1122 ?S
    MY-SET NEXT-CODEWORD 0 ?S
}T

T{ ." inserting codewords into set" CR
    MY-SET CELL+ SETSIZE ERASE ( eliminate all codewords from set )
    1234 MY-SET INSERT-CODEWORD
    2341 MY-SET INSERT-CODEWORD
    6622 MY-SET INSERT-CODEWORD
    MY-SET START-SET
    MY-SET NEXT-CODEWORD 1234 ?S
    MY-SET NEXT-CODEWORD 2341 ?S
    MY-SET NEXT-CODEWORD 6622 ?S
    MY-SET NEXT-CODEWORD 0 ?S
}T

BYE
